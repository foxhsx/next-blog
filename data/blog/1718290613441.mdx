...
大家平常在工作中或者生活中都会有制定目标的需求，但是制定目标之后怎么将其分解为一个个小的 case、并为这些小任务制定计划，然后按照计划加以执行等等，一套组合拳下来往往需要耗费大把的精力。

今天要介绍的 AgentGPT，是 Github 上拥有 30.4k stars 的开源项目。官方介绍是这样的：
...
在名称上输入 AI 的名称，在目标一栏输入一个目标，按下回车或者点击左侧的三角按钮之后它就会帮我们完成这个目标。比如我们想要学好 React，熟练掌握它的语法和用法，知道它的一些底层逻辑。

![](https://note.ihsxu.com/api/imgs/1718808957799.webp)

在很短的时间内，就列出了 8 个 task，并完成了其中的 5 个，为啥剩下的三个没完成？因为现在使用的是免费的 AgentGPT-3.5，只能支持到这里了，想要完整的功能，请开通付费🐶

在已完成的这些 task 中，每个 task 都会给出详细的步骤和内容：
...
在使用 docker 部署前，我们按照文档根据不同的系统拉取项目并运行脚本。
...
在拉取项目之后，我们使用 VSCode 打开 AgentGPT 代码，然后我们需要修改三个点：
