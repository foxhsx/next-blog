...
draft: false
summary: 手写数组的 push 方法，那我们第一步想到的应该是数组本身的 push 方法，push方法将一个或者多个元素添加到数组的末尾，并返回该数组的新长度。
---

> 实际上这道题意义在哪里？它肯定不是说让你去背数组 push 方法的底层是怎么实现的，而是考察你的整体编程思想和思维逻辑。如果你没有自己写过 push 方法，那么就看你的临场发挥，能否在短时间内有一个思路出来，并表述出自己的思路和想法；当然，如果你已经看过这方面的内容，那就更没有什么问题了。
