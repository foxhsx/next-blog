...
draft: false
summary: 什么是继承对于继承这个概念来说大家都不陌生，在现实中有儿子继承父业，孙子继承了爷爷的财产等等。那么在编程中，我们熟知的继承其实主要来自于面向对象编程。而很多的面向对象语言都支持两种继承：接口继承和实现继承。前者是只继承方法签名，后者继承实际的方法。继承的主要作用是将一些抽象的模块抽离出来，作为一个父类使用，这样便于实际编程中代码的可复用性和可维护性，也从一定程度上减少代码冗余，使得代码更加的简洁整齐。在 JavaScript 中接口继承是不可能实现的，因为函数没有签名。所以实现继承成了 JavaScr_js 类继承接口
---
...
![](https://note.ihsxu.com/api/imgs/1694273681847.webp)

试想一下，假如**原型是另一个类型的实例**呢？是不是就会有：

> 原型本身有一个内部指针指向另一个原型，相应地另一个原型也有一个指针指向另一个构造函数。这样重复下去直到 null，就形成了在实例和原型之间的一条原型链。

![在这里插入图片描述](https://img-blog.csdnimg.cn/2106acd12b2f4c0790c77d4aef34382f.jpg?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBAbGVhcm5pbmdfSA==,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center "在这里插入图片描述")![](https://note.ihsxu.com/api/imgs/1694314174993.webp)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/0716089daf1f4be98e657d6b50a8bd2d.jpg?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBAbGVhcm5pbmdfSA==,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center "在这里插入图片描述")![](https://note.ihsxu.com/api/imgs/1694314335089.webp)
...
![](https://note.ihsxu.com/api/imgs/1694317053038.webp)
...
![](https://note.ihsxu.com/api/imgs/1694403506321.webp)
...
这里就只调用了一次父类函数，避免了重复调用的问题，提高了效率。寄生式组合继承可以算是**引用类型继承的最佳模式**。
...
5.  如果没有定义类构造函数，在实例化派生类时会调用 super() ，而且会传入所有派生类的参数
6.  不能在 super() 之前引用 this
